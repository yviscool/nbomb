...
import { PATH_METADATA, METHOD_METADATA, REST_MAP, RequestMethod } from './constants';

export class MiddlewaresConsumer {

  static router: any;
  static routers: any;
  static globalPrefix: any;
  ...
  static init(routers, globalPrefix) {
    this.storges =  {
  ...
  static reflectRequestMethod(proto, method) {
    return Reflect.getMetadata(METHOD_METADATA, proto, method);
  }
  ...
  static scanForPaths({ controller, prefix }) {
    ...
    return Object
      ...
      .filter(method => this.reflectMethodPath(proto, method) || Object.keys(REST_MAP).some(key => proto[key]))
      .reduce((routerPathAndMethod, method) => {
        const methodPath = this.reflectMethodPath(proto, method);
        const requestMethod = this.reflectRequestMethod(proto, method);
